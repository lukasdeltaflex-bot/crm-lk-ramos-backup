...
export function CustomerAttachmentUploader({
  ...
  const handleUpload = (file: File) => {
    if (isReadOnly || !customerId) return;
    const filePath = `customers/${userId}/${customerId}/${file.name}`;
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          ...
          toast({ title: 'Documento Adicionado', description: `O arquivo ${file.name} foi salvo na central.` });
          setUploadingFiles(prev => {
            const newUploading = { ...prev };
            delete newUploading[file.name];
            return newUploading;
