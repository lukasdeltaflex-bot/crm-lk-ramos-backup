        ...
        >
            <div className="flex flex-col h-full justify-center">
                ...
                >
                    ...
                    <div className={cn("overflow-hidden font-black text-[12px] uppercase tracking-wider text-foreground leading-tight flex items-center gap-1", isSortable && "cursor-pointer", header.column.id === 'Ações' && "text-right pr-2", isSelect && "justify-center w-full pr-0")}>
...
export const getColumns = ({ onEdit, onStatusUpdate }: any): ColumnDef<ProposalWithCustomer>[] => [
  ...
  { id: 'Comissão', accessorKey: 'commissionValue', header: 'Comissão', cell: ({ row, table }) => { const isPriv = (table.options.meta as any)?.isPrivacyMode; return (<div className="text-right font-black text-sm">{isPriv ? '•••••' : formatCurrency(row.original.commissionValue)}</div>) }, size: 120 },
